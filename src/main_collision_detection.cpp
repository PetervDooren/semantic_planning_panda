 ...
 *
 * Move the robot down until a collision is detected.
 */
...
int main(int argc, char** argv) {
        ...
        while (running) {
            ...
            if (print_data.mutex.try_lock()) {
                if (print_data.has_data) {
                    // convert to Eigen
                    Eigen::Map<const Eigen::Matrix<double, 6, 7>> jacobian(print_data.jacobian.data());
                    Eigen::Map<const Eigen::Matrix<double, 7, 1>> dq(print_data.robot_state.dq.data());
    ...
    */
        ...
        std::function<franka::Torques(const franka::RobotState&, franka::Duration)>
                ...
                [&print_data, &model, &velocityControl](
                const franka::RobotState& state, franka::Duration period) -> franka::Torques {
            // Read current coriolis terms from model.

            Eigen::Vector3d desired_velocity = {0, 0, -0.1};
            std::array<double, 7> tau_d_input = velocityControl.controlLaw(state, period, desired_velocity);
            ...
            if (print_data.mutex.try_lock()) {
                ...
                print_data.jacobian = model.zeroJacobian(franka::Frame::kEndEffector, state);
                print_data.mutex.unlock();
            }
